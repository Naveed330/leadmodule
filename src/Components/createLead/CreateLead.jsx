...
const CreateLead = ({ setModal2Open, modal2Open, fetchLeadsData }) => {
    ...
    const checkClientPhone = async (completePhoneNumber) => {
        ...
        try {
            ...
            if (responseData && responseData.client.phone === completePhoneNumber) {
                console.log(completePhoneNumber, responseData && responseData.client.phone, 'completePhoneNumber')
    ...
    return (
        <>
            ...
            >
                <Modal.Body>
                    <div className='moveleadsmessagerequest' >
                        <FaCheck style={{ fontSize: '40px', color: 'white' }} />
                    </div>
                    <p className='text-center mt-3' style={{ fontWeight: '600' }} >
                        Request Send SuccessFully from {apiData && apiData?.branch && apiData?.branch?.name} to {branch}
                    </p>
                </Modal.Body>
                <Modal.Footer>
